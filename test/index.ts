...
test('Able to initialize a member', async (t) => {
...
test('Able to add a member by ID', async (t) => {
  const member = await Member.create()
  ...
  t.equal((await member.feed.get(1)).type, 'request', 'auto-generated request')
...
test('Able to add a member by ID and sync', async (t) => {
  const member = await Member.create()
  const other = await Member.create({ initiator: member.id })
  ...
  t.deepEqual(
    other.knownMembers,
    expectedMembers,
    'await Member.create saw itself added to list'
  )
})

test('Happy path of adding several members together', async (t) => {
  ...
  const f = await Member.create({ id: 'f', initiator: 'a' })
...
test('Able to initialize a bunch of members', async (t) => {
...
test('Process request by syncing one peer at a time', async (t) => {
  const members = await initializeMembers(5, { knowEachOther: true })
...
test('Only two members remove each other', async (t) => {
...
test('Multiple requests get treated one at a time', async (t) => {
  const [a, b] = await initializeMembers(3, { knowEachOther: true })

  const e = await Member.create({ id: 'e' })
  const f = await Member.create({ id: 'f' })
  const g = await Member.create({ id: 'g' })
...
test('Two members do an add at once', async (t) => {
  const [a, b, c, d, e] = await initializeMembers(5, { knowEachOther: true })

  const f = await Member.create({ id: 'f', initiator: 'a' })
  const g = await Member.create({ id: 'g', initiator: 'a' })
...
test('Concurrent requests should resolve to the same state on all members', async (t) => {
  const [a, b, c] = await initializeMembers(3, { knowEachOther: true })

  // Set up two external peers
  const d = await Member.create({ id: 'd', initiator: a.id })
  const e = await Member.create({ id: 'e', initiator: a.id })
...
async function initializeMembers (n: number, { knowEachOther }: { knowEachOther: boolean }): Promise<Member[]> {
  ...
  const initiator = await Member.create({ id: 'a' })
